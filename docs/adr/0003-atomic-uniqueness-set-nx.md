...
`EXISTS`/`GET` check, then both write; last writer wins, and the first user's
...
- The whole user must live under a single key (JSON blob) rather than
  a Redis hash plus separate index key. That's acceptable at this record
  size, and it removes an entire class of index-desync bugs.
