...
| Supply chain                             | Lockfile, minimal dependency set, `npm audit` gate in CI, CodeQL SAST, gitleaks secret scan (full history), Trivy scan of the production image, dependency-review gate on PRs, checkov over Dockerfile/Actions — findings surface as SARIF in the Security tab — plus Dependabot (npm / actions / docker / terraform) | `.github/`                                                      |
| Container compromise blast radius        | Multi-stage image: no devDependencies, no TS sources, **no package manager** (npm/corepack stripped from the final stage, so a compromised container cannot install tooling), runs as the unprivileged `node` user (CI asserts uid ≠ 0)                                                                               | `Dockerfile`, `ci.yml`                                          |
